...
@C0463SafeParcelable.Class(creator = "ActivityTransitionResultCreator")
@C0463SafeParcelable.Reserved({1000})
public class ActivityTransitionResult extends C0460AbstractSafeParcelable {
    public static final Parcelable.Creator<ActivityTransitionResult> CREATOR;
    @C0463SafeParcelable.Field(getter = "getTransitionEvents", id = 1)
    ...
    public ActivityTransitionResult(@C0463SafeParcelable.Param(id = 1) List<ActivityTransitionEvent> list) {
        List<ActivityTransitionEvent> list2 = list;
        Object checkNotNull = C0446Preconditions.checkNotNull(list2, "transitionEvents list can't be null.");
        List<ActivityTransitionEvent> list3 = list2;
        List<ActivityTransitionEvent> list4 = list3;
        if (!list3.isEmpty()) {
            for (int i = 1; i < list4.size(); i++) {
                C0446Preconditions.checkArgument(list4.get(i).getElapsedRealTimeNanos() >= list4.get(i + -1).getElapsedRealTimeNanos());
            }
        }
    ...
    public static ActivityTransitionResult extractResult(Intent intent) {
        ...
        return (ActivityTransitionResult) C0464SafeParcelableSerializer.deserializeFromIntentExtra(intent2, "com.google.android.location.internal.EXTRA_ACTIVITY_TRANSITION_RESULT", CREATOR);
    }
    ...
    public void writeToParcel(Parcel parcel, int i) {
        ...
        C0462SafeParcelWriter.writeTypedList(parcel2, 1, getTransitionEvents(), false);
