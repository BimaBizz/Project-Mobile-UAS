...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state, View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (this.mOrientation == 0) {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(glp.getSpanIndex(), glp.getSpanSize(), spanGroupIndex, 1, (this.mSpanCount <= 1 || glp.getSpanSize() != this.mSpanCount) ? DEBUG : true, DEBUG));
        } else {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(spanGroupIndex, 1, glp.getSpanIndex(), glp.getSpanSize(), (this.mSpanCount <= 1 || glp.getSpanSize() != this.mSpanCount) ? DEBUG : true, DEBUG));
        }
    }
    ...
    private void ensureAnchorIsInCorrectSpan(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.AnchorInfo anchorInfo, int itemDirection) {
        ...
        RecyclerView.State state2 = state;
        LinearLayoutManager.AnchorInfo anchorInfo2 = anchorInfo;
        boolean layingOutInPrimaryDirection = itemDirection == 1 ? true : DEBUG;
    ...
    public void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult layoutChunkResult) {
        ...
        boolean layingOutInPrimaryDirection = layoutState2.mItemDirection == 1 ? true : DEBUG;
        int count = 0;
        ...
        for (int i = 0; i < count; i++) {
            ...
            measureChild(view2, otherDirSpecMode, DEBUG);
        ...
        for (int i3 = 0; i3 < count; i3++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view4) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    wSpec = getChildMeasureSpec(totalSpaceInOther, Declaration.MODULE_REFERENCE, horizontalInsets, lp.width, DEBUG);
                    ...
                    hSpec = getChildMeasureSpec(totalSpaceInOther, Declaration.MODULE_REFERENCE, verticalInsets, lp.height, DEBUG);
                }
    ...
    private void measureChild(View view, int i, boolean z) {
        ...
        if (this.mOrientation == 1) {
            wSpec = getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, horizontalInsets, lp.width, DEBUG);
            hSpec = getChildMeasureSpec(this.mOrientationHelper.getTotalSpace(), getHeightMode(), verticalInsets, lp.height, true);
        } else {
            hSpec = getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, verticalInsets, lp.height, DEBUG);
    ...
    public View onFocusSearchFailed(View view, int i, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        boolean preferLastSpan = (this.mOrientation != 1 || !isLayoutRTL()) ? DEBUG : true;
                ...
                if ((!candidate.hasFocusable() || focusableWeakCandidate != null) && (candidate.hasFocusable() || unfocusableWeakCandidate != null)) {
                    ...
                    if (candidate.hasFocusable()) {
                        if (overlap > focusableWeakCandidateOverlap) {
                            ...
                            if (preferLastSpan == (candidateStart > focusableWeakCandidateSpanIndex ? true : DEBUG)) {
                                assignAsWeek = true;
                            }
                        }
                    } else if (focusableWeakCandidate == null && isViewPartiallyVisible(candidate, DEBUG, true)) {
                        if (overlap > unfocusableWeakCandidateOverlap) {
                            assignAsWeek = true;
                        } else if (overlap == unfocusableWeakCandidateOverlap) {
                            if (preferLastSpan == (candidateStart > unfocusableWeakCandidateSpanIndex ? true : DEBUG)) {
                                assignAsWeek = true;
                            }
                        }
                    }
