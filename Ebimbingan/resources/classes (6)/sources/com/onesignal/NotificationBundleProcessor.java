...
class NotificationBundleProcessor {
    ...
    private static void unMinifyButtonsFromBundle(Bundle bundle) {
        ...
        if (gcmBundle.containsKey(PUSH_MINIFIED_BUTTONS_LIST)) {
            try {
                new JSONObject(gcmBundle.getString(OSNotificationFormatHelper.PAYLOAD_OS_ROOT_CUSTOM));
                ...
                for (int i = 0; i < buttons.length(); i++) {
                    ...
                    Object remove = button.remove(PUSH_MINIFIED_BUTTON_TEXT);
                    if (button.has("i")) {
                    ...
                    JSONObject put2 = button.put("text", buttonText);
                    if (button.has(PUSH_MINIFIED_BUTTON_ICON)) {
                        JSONObject put3 = button.put("icon", button.getString(PUSH_MINIFIED_BUTTON_ICON));
                        Object remove3 = button.remove(PUSH_MINIFIED_BUTTON_ICON);
                    }
                }
                ...
                gcmBundle.putString(OSNotificationFormatHelper.PAYLOAD_OS_ROOT_CUSTOM, customJSON.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private static void setActionButtons(OSNotificationPayload oSNotificationPayload) throws Throwable {
        ...
        if (notification.additionalData != null && notification.additionalData.has("actionButtons")) {
            ...
            for (int i = 0; i < jsonActionButtons.length(); i++) {
                ...
                actionButton2.text = jsonActionButton.optString("text", (String) null);
    ...
    static JSONObject getCustomJSONObject(JSONObject jsonObject) throws JSONException {
        JSONObject jsonObject2;
        new JSONObject(jsonObject.optString(OSNotificationFormatHelper.PAYLOAD_OS_ROOT_CUSTOM));
        return jsonObject2;
    }
