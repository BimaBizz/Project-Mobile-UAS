...
public final class zzaq {
    ...
    public static void zza(Context context, FirebaseAuth firebaseAuth) {
        ...
        Object checkNotNull2 = C0446Preconditions.checkNotNull(firebaseAuth2);
    ...
    public static void zza(Context context, zzfm zzfm, String str, @Nullable String str2) {
        ...
        SharedPreferences.Editor editor = edit;
        SharedPreferences.Editor putString = edit.putString("verifyAssertionRequest", C0464SafeParcelableSerializer.serializeToString(zzfm));
        ...
        SharedPreferences.Editor putLong = editor.putLong("timestamp", C0597DefaultClock.getInstance().currentTimeMillis());
        boolean commit = editor.commit();
    }

    public static void zza(Context context, C0236Status status) {
    ...
    public final void zzg(FirebaseAuth firebaseAuth) {
        C0236Status status;
        FirebaseAuth firebaseAuth2 = firebaseAuth;
        Object checkNotNull = C0446Preconditions.checkNotNull(firebaseAuth2);
        ...
        if (firebaseAuth2.zzcu().getName().equals(sharedPreferences.getString("firebaseAppName", ""))) {
            if (sharedPreferences2.contains("verifyAssertionRequest")) {
                zzfm zzfm = (zzfm) C0464SafeParcelableSerializer.deserializeFromString(sharedPreferences2.getString("verifyAssertionRequest", ""), zzfm.CREATOR);
                ...
                switch (z) {
                    case false:
                        this.zzut = firebaseAuth2.signInWithCredential(zzf.zza(zzfm));
                        break;
                    case true:
                        if (!firebaseAuth2.getCurrentUser().getUid().equals(string3)) {
                            ...
                            this.zzut = firebaseAuth2.getCurrentUser().linkWithCredential(zzf.zza(zzfm));
                            break;
                        }
                    case true:
                        if (!firebaseAuth2.getCurrentUser().getUid().equals(string3)) {
                            ...
                            this.zzut = firebaseAuth2.getCurrentUser().reauthenticateAndRetrieveData(zzf.zza(zzfm));
                            break;
                        }
                    default:
                        this.zzut = null;
                ...
                new C0236Status(sharedPreferences2.getInt("statusCode", 17062), sharedPreferences2.getString("statusMessage", ""));
                this.zzuu = sharedPreferences2.getLong("timestamp", 0);
                zza(sharedPreferences2);
    ...
    public final Task<AuthResult> zzfo() {
        if (C0597DefaultClock.getInstance().currentTimeMillis() - this.zzuu < zzuq) {
            return this.zzut;
        }
        return null;
    }
